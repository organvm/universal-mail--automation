...
class EmailMessage:
    ...
    Attributes:
        ...
        headers: Bulk-signal headers captured at list time, as a {name: value} map
            (lower-cased names). Only the mailing-list / bulk / auto markers are kept
            (list-unsubscribe, list-id, list-post, precedence, auto-submitted) so the
            classifier can suppress bulk mail from the reply-owed rung. Empty when a
            provider does not (or cannot cheaply) supply headers — fail-open.
    """
    id: str
    sender: str
    subject: str
