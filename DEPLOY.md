...
Code/build blockers are cleared. The canonical deployable artifact is the
Docker/FastAPI app. The optional Cloudflare share demo separately uses the Next
static export generated by `npm --prefix web run build`.

Exactly what remains outside the repo:

1. Pick the first production host: Render blueprint, Fly.io, GHCR image, or another
   Docker host.
2. Set runtime secrets on that host: mailbox provider credentials, Stripe secrets
   for paid checkout, `RECEIPT_SIGNING_KEY`, and `MCP_ALLOWED_HOSTS=<host>`.
3. If shipping the Cloudflare share Worker, set `CLOUDFLARE_API_TOKEN` in the lane
   or GitHub Actions secrets; the app/container deploy path does not need it.
4. Review and merge the chosen release PR(s). The current deploy-ready PR is #93;
   PRs #94 and #95 are adjacent readiness/security work and should be reviewed
   separately. Remote merge remains a human-gated action.
5. Register OAuth apps for true customer-owned mailbox connections. Until then,
   hosted deploys are single-tenant/operator-credentialed.
